...
SELECT * FROM company_divisions /*Вывели 5 строк подразделений компании*/
LIMIT 5;
SELECT * FROM company_regions /*Вывели 5 строк регионов компании*/
LIMIT 5;
SELECT * FROM staff /*Вывели 5 строк персонала*/
LIMIT 5;
SELECT COUNT(*) FROM staff; /*Количество сотрудников в компании*/
SELECT gender, COUNT(*) AS total_employees /*Распределение сотрудников по полу*/
FROM staff
GROUP BY gender; 
SELECT department, COUNT(*) AS total_employee /*Вывели количество сотрудников в каждом отделе*/
FROM staff
GROUP BY department
ORDER BY department;
SELECT DISTINCT(department) /*Вывели все имеющиеся отделы компании*/
FROM staff
ORDER BY department;
SELECT MAX(salary) AS Max_Salary, MIN(salary) AS Min_Salary /*Определили самую высокую и низкую зарплату сотрудников*/
FROM staff;
SELECT gender, MIN(salary) As Min_Salary, MAX(salary) AS Max_Salary, AVG(salary) AS Average_Salary /*Определили макс, мин и среднюю зарплату по полу*/
FROM staff
GROUP BY gender;
SELECT SUM(salary) /*Определили общую зарплату всех сотрудников*/
FROM staff;
SELECT /*Вывели макс, мин и среднюю зарплату по отделам и кол-во сотрудников в них*/
	department, 
...
GROUP BY department
ORDER BY 4 DESC;
SELECT /*Вывели макс, мин и среднюю зарплату по отделам, кол-во сотрудников в них,
 разницу между зарплатами и определили, что наибольшее распространение имеет отдел здравоохранения*/
	department, 
	MIN(salary) As Min_Salary, 
	MAX(salary) AS Max_Salary, 
...
GROUP BY department
ORDER BY 6 DESC;
SELECT department, salary /*Узнали зарплату отдела здравоохранения*/
FROM staff
WHERE department LIKE 'Health'
ORDER BY 2 ASC;
CREATE VIEW health_dept_earning_status /*Создали представление для отдела здравоохранения, чтобы увидеть статус зарплаты*/
AS 
	SELECT 
		CASE
	...
	FROM staff
	WHERE department LIKE 'Health';
    SELECT earning_status, COUNT(*) /*Узнали статус зарплаты отдела здравоохранения*/
FROM health_dept_earning_status
GROUP BY 1;
SELECT department, salary /*Узнали о зарплате отдела экстерьера*/
FROM staff
WHERE department LIKE 'Outdoors'
ORDER BY 2 ASC;
CREATE VIEW outdoors_dept_earning_status /*Создали представление для отдела экстерьера, чтобы увидеть статус зарплаты*/
AS 
	SELECT 
		CASE
			WHEN salary >= 100000 THEN 'high earner'
...
SELECT DISTINCT(department) /*Вывели все имеющиеся отделы компании, без повторения*/
FROM staff
ORDER BY department;
SELECT DISTINCT(UPPER(department)) /*Вывели все имеющиеся отделы компании с большой буквы*/
FROM staff
ORDER BY 1;
SELECT DISTINCT(LOWER(department)) /*Вывели все имеющиеся отделы компании с маленькой буквы*/
FROM staff
ORDER BY 1;
SELECT /*Сделали удаление не нужных отступов*/
	TRIM('    data sciece rocks !    ');
SELECT /*Узнали длину строки*/
	LENGTH(TRIM('    data sciece rocks !    '));
SELECT DISTINCT(job_title) /*У различных должностей определили помощника*/
FROM staff
WHERE job_title LIKE '%Assistant%'
ORDER BY 1;
SELECT /*Узнали какие должности являются помощниками, а какие нет*/
	DISTINCT(job_title),
...
SELECT  /*Извлекли строку с 3 буквами начиная с пятой*/
	SUBSTRING('abcdefghikl' FROM 5 FOR 3) as sub_string;
SELECT /*Извлекли строку с буквами начиная с пятой*/
	SUBSTRING('abcdefghikl' FROM 5) as sub_string;
    
SELECT job_title /*Отобразили помощников компании*/
FROM staff
WHERE job_title LIKE 'Assistant%';
SELECT /*Извлекли категории помощников*/
	SUBSTRING(job_title FROM LENGTH('Assistant')+1) AS job_category,
	job_title
FROM staff
WHERE job_title LIKE 'Assistant%';
SELECT /*Извлекли категории помощников без дубликатов*/
	DISTINCT(SUBSTRING(job_title FROM LENGTH('Assistant')+1)) AS job_category,
	job_title
FROM staff
WHERE job_title LIKE 'Assistant%';

SELECT /*Узнали сравнение между средней и обычной зарплатой человека по отделам*/
	s.last_name,s.salary,s.department,
	(SELECT ROUND(AVG(salary),2)
	 FROM staff s2
	 WHERE s2.department = s.department) AS department_average_salary
FROM staff s;
CREATE VIEW vw_salary_comparision_by_department /*Создали представление, чтобы узнать сколько людей зарабатывают выше/ниже по отделам*/
AS
	SELECT 
	s.department,
	(
...
SELECT department, is_higher_than_dept_avg_salary, COUNT(*) AS total_employees /*Отобразили кол-во сотрудников по условиям данным выше*/
FROM vw_salary_comparision_by_department
GROUP BY 1,2;

SELECT department, ROUND(AVG(salary),2) AS average_salary /*Узнаем среднюю зарплату руководителей по каждому отделу*/
FROM staff
WHERE salary >= 100000
GROUP BY department
ORDER BY 2 DESC;
SELECT last_name, department, salary /*Узнаем кто больше всего зарабатывает в компании*/
FROM staff
WHERE salary = (
	SELECT MAX(s2.salary)
	FROM staff s2);
SELECT s.department, s.last_name, s.salary /*Узнаем кто больше зарабатывает по отделам*/
FROM staff s
WHERE s.salary = (
	SELECT MAX(s2.salary)
	FROM staff s2
...
SELECT * FROM company_divisions; /*Отделы подразделений компании*/
SELECT s.last_name, s.department, cd.company_division /*Полная информация о содрудниках, подразделениях и отделах*/
FROM staff s
JOIN company_divisions cd
	ON cd.department = s.department;
SELECT s.last_name, s.department, cd.company_division /*Нашли сотрудников, у которых нет подразделения*/
FROM staff s
LEFT JOIN company_divisions cd
	ON cd.department = s.department;
SELECT s.last_name, s.department, cd.company_division /*Отобразили всех сотрудников без подразделений*/
FROM staff s
LEFT JOIN company_divisions cd
	ON cd.department = s.department
WHERE company_division IS NULL;

CREATE VIEW vw_staff_div_reg AS /*Создали представление сотрудников с регионами и подразделениями*/
...
SELECT COUNT(*) /*Узнали кол-во сотрудников*/
FROM vw_staff_div_reg;
SELECT company_regions, COUNT(*) AS total_employees /*Узнали кол-во сотрудников в каждом регионе*/
FROM vw_staff_div_reg
GROUP BY 1
ORDER BY 1;

CREATE OR REPLACE VIEW vw_staff_div_reg_country AS /*Создали еще одно представление сотрудников по странам*/
	SELECT s.*, cd.company_division, cr.company_regions, cr.country
	FROM staff s
	LEFT JOIN company_divisions cd ON s.department = cd.department
	LEFT JOIN company_regions cr ON s.region_id = cr.region_id;
SELECT company_regions, country, COUNT(*) AS total_employees /*Кол-во сотрудников по региона и странам*/
FROM vw_staff_div_reg_country
GROUP BY 
	company_regions, country
ORDER BY country, company_regions;

SELECT /*Зарплата сотрудников в сравнении со средней зарплатой отдела*/
	department, 
	last_name, 
	salary,
	AVG(salary) OVER (PARTITION BY department)
FROM staff;
SELECT /*Зарплата сотрудников в сравнении с максимальной зарплатой отдела*/
	department, 
	last_name, 
	salary,
	MAX(salary) OVER (PARTITION BY department)
FROM staff;
SELECT /*Зарплата сотрудников в сравнении с минимальной зарплатой отдела*/
	company_regions, 
	last_name, 
	salary,
...
SELECT /*Таблица отражает зарплату сотрудников по отделам с зарплатой которая находится в начале*/
	department,
	last_name,
	salary,
	FIRST_VALUE(salary) OVER (PARTITION BY department ORDER BY salary DESC)
FROM staff;
SELECT /*Сравнение зарплаты сотрудника с зарплатой того сотрудника кто находится в начале по первой букве*/
	department,
	last_name,
	salary,
...
SELECT /*Присвоили ранг к кадому отделу по уменьшению зарплаты*/
	department,
	last_name,
	salary,
	RANK() OVER (PARTITION BY department ORDER BY salary DESC)
FROM staff;
SELECT /*Сравнили зарплату каждого с предыдущей строкой*/
	department,
	last_name,
	salary,
...
SELECT /*Сравнили зарплату со следующей срокой предыдущей строки*/
	department,
	last_name,
	salary,
	LEAD(salary) OVER(PARTITION BY department ORDER BY salary DESC)
FROM staff;
SELECT /*Отсортировали каждого сотрудника в зависимости от убывающей зарплата по отделам*/
	department,
	last_name,
	salary,
