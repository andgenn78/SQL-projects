...
GO
	/* ======================= TABLES ========================*/
	-- комментарии:
	-- NOT NULL - заставляет столбец не принимать значения NULL, что означает, что вы не можете вставить или обновить запись без добавления значения в это поле
	-- PRIMARY KEY - это параметр, который устанавливается для однозначной идентификации той или иной записи в таблице
	-- FOREIGN KEY REFERENCES - создание связи между таблицами
	-- CONSTRAINT - создание ограничения
	...
	-- ON DELETE CASCADE - используется для автоматического удаления строк из дочерней таблицы при удалении строк из родительской таблицы
	-- SELECT * FROM - возвращает все строки и столбцы из выбранной таблицы
	-- * - это подстановочный знак, обозначающий 'все'
	...
	CREATE TABLE tbl_book (
		book_BookID INT PRIMARY KEY NOT NULL IDENTITY (1,1), --IDENTITY - это свойство, которое необходимо для создание автоматической нумерации 
